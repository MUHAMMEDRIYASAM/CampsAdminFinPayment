...
public class FIN_PaymentProcess implements org.openbravo.scheduling.Process {
	...
	public static void doProcessPayment(FIN_Payment payment, String strAction, String paymentDate, String comingFrom)
			throws OBException {
	...
	@Deprecated
	public static void doProcessPayment(FIN_Payment payment, String strAction, Boolean isPosOrder, String paymentDate,
			String comingFrom) throws OBException {
		doProcessPayment(payment, strAction, paymentDate, comingFrom);
	}

	public void processPayment(FIN_Payment payment, String strAction, String paymentDate, String comingFrom,
			String selectedCreditLineIds, boolean doFlush) throws OBException {
		...
		try {
			...
			if (strAction.equals("P") || strAction.equals("D")) {
				...
				// FIXME: added to access the FIN_PaymentSchedule and FIN_PaymentScheduleDetail
				// tables to be
					...
					for (FIN_PaymentDetail paymentDetail : paymentDetails) {
						for (FIN_PaymentScheduleDetail paymentScheduleDetail : paymentDetail
							...
							if (paymentScheduleDetail.getInvoicePaymentSchedule() != null) {
								final Invoice invoice = paymentScheduleDetail.getInvoicePaymentSchedule().getInvoice();
								invoiceDocNos.add(FIN_Utility.getDesiredDocumentNo(payment.getOrganization(), invoice));
							}
				...
				try {
					...
					reversedPayment
							.setDocumentNo("*R*" + FIN_Utility.getDocumentNo(payment.getDocumentType(), "FIN_Payment"));
					...
					reversedPayment.setCreationDate(new Date());
					reversedPayment.setUpdated(new Date());
					...
					for (FIN_PaymentDetail pd : payment.getFINPaymentDetailList()) {
						...
						for (FIN_PaymentScheduleDetail psd : pd.getFINPaymentScheduleDetailList()) {
							...
							reversedPaymentSchedDetail.setDoubtfulDebtAmount(psd.getDoubtfulDebtAmount().negate());
					...
					for (FIN_Payment_Credit pc : credits) {
						...
						pc.getCreditPaymentUsed()
								.setUsedCredit(pc.getCreditPaymentUsed().getUsedCredit().add(pc.getAmount().negate()));
				...
				FIN_PaymentProcess fpp = new FIN_PaymentProcess();
				fpp.processPayment(reversedPayment, newStrAction, paymentDate, comingFrom, selectedCreditLineIds, true);
				log4j.info("New Payment DocNo: " + reversedPayment.getDocumentNo());
				return;
				// ***********************
				// Reactivate Payment
				// ***********************
				...
				try {
					...
					for (FIN_PaymentDetail paymentDetail : paymentDetails) {
						...
						// b) if we are reactivating a payment (deleting lines only) and we don't come
						// from
						...
						for (FIN_PaymentScheduleDetail psd : removedPDS) {
							...
							if (BigDecimal.ZERO.compareTo(psd.getAmount()) == 0
								...
								// Before deleting the PSD, we must delete any payment proposal line linked to
								// it
							...
							// Delete any payment proposal line linked to the PSD if we are reactivating a
							// payment
							// (deleting lines only), we haven't removed it in a previous step and we don't
							// come
							// from payment proposal reactivation process
							if (strAction.equals("R") && proposalLinesRemoved == 0 && !StringUtils.equals(comingFrom,
							...
							for (final FIN_PaymentScheduleDetail paymentScheduleDetail : paymentDetail
								...
								if (paymentScheduleDetail.getInvoicePaymentSchedule() != null) {
									...
									if (outStandingAmt.compareTo(BigDecimal.ZERO) != 0) {
										final FIN_PaymentScheduleDetail mergedScheduleDetail = dao
												.getNewPaymentScheduleDetail(paymentScheduleDetail
														.getOrderPaymentSchedule().getOrder().getOrganization(),
														outStandingAmt);
										mergedScheduleDetail.setOrderPaymentSchedule(
												paymentScheduleDetail.getOrderPaymentSchedule());
										mergedScheduleDetail
	...
	/**
	 * Method used to update the credit used when the user doing invoice processing
	...
	private static boolean hasTransaction(FIN_Payment payment) {
		OBCriteria<FIN_FinaccTransaction> transaction = OBDal.getInstance().createCriteria(FIN_FinaccTransaction.class);
	...
	private void updateUsedCredit(FIN_Payment newPayment, String selectedCreditLineIds) {
		if (newPayment.getFINPaymentCreditList().isEmpty()) {
			// We process the payment from the Payment In/Out window (not from the Process
			// Invoice flow)
			...
			for (FIN_Payment creditPayment : creditPayments) {
				BigDecimal availableAmount = creditPayment.getGeneratedCredit().subtract(creditPayment.getUsedCredit());
	...
	private List<ConversionRateDoc> getConversionRateDocument(FIN_Payment payment) {
		...
		try {
			...
			obc.add(Restrictions.eq(ConversionRateDoc.PROPERTY_TOCURRENCY, payment.getAccount().getCurrency()));
	...
	private List<ConversionRateDoc> getConversionRateDocumentForInvoice(Invoice invoice) {
		...
		try {
			...
			obc.add(Restrictions.eq(ConversionRateDoc.PROPERTY_TOCURRENCY, invoice.getBusinessPartner().getCurrency()));
	 ...
	 * @throws OBException
	 */
	private void processTransaction(String strAction, FIN_FinaccTransaction transaction) throws OBException {
		FIN_TransactionProcess.doTransactionProcess(strAction, transaction);
	}

	public boolean getDocumentConfirmation(ConnectionProvider conn, String strRecordId) {
		// Checks if this step is configured to generate accounting for the selected
		// financial account
		boolean confirmation = false;
		OBContext.setAdminMode();
		try {
			FIN_Payment payment = OBDal.getInstance().get(FIN_Payment.class, strRecordId);
			OBCriteria<FinAccPaymentMethod> obCriteria = OBDal.getInstance().createCriteria(FinAccPaymentMethod.class);
			...
			List<FIN_FinancialAccountAccounting> accounts = payment.getAccount().getFINFinancialAccountAcctList();
			for (FIN_FinancialAccountAccounting account : accounts) {
				...
				if (payment.isReceipt()) {
					if (("INT").equals(lines.get(0).getUponReceiptUse())
						...
						confirmation = true;
					else if (("DEP").equals(lines.get(0).getUponReceiptUse()) && account.getDepositAccount() != null)
						confirmation = true;
					else if (("CLE").equals(lines.get(0).getUponReceiptUse())
							&& account.getClearedPaymentAccount() != null)
						confirmation = true;
				} else {
					if (("INT").equals(lines.get(0).getUponPaymentUse()) && account.getFINOutIntransitAcct() != null)
						confirmation = true;
					else if (("WIT").equals(lines.get(0).getUponPaymentUse()) && account.getWithdrawalAccount() != null)
						confirmation = true;
					else if (("CLE").equals(lines.get(0).getUponPaymentUse())
							&& account.getClearedPaymentAccountOUT() != null)
						confirmation = true;
				}
				// For payments with Amount ZERO always create an entry as no transaction will
				// be created
	...
	private BigDecimal getConversionRate(String strOrgId, String strFromCurrencyId, String strToCurrencyId,
		...
		final ConversionRate conversionRate = FIN_Utility.getConversionRate(fromCurrency, toCurrency, conversionDate,
				organization);
	...
	/**
	 * Removes the Payment Proposal Lines linked to the Payment Schedule Detail. If
	 * the payment proposal is already processed, the record is unprocessed first,
	 * then affected lines are deleted, and finally it gets back to processed
	 * status. This is done to avoid trigger validation errors.
	 * 
	 * A common scenario where you want to delete a payment proposal line is when
	 * the payment schedule detail has been deleted
	 * 
	 * @param psd
